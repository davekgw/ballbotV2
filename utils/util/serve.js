...
export default async(serve) => {
	try {
		...
		* By Bolaxd
		*/
		serve.ments = async (query) => query.match('@') ? [...query.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + q.idwa):[]
		/* 
		* Create jid at id
		* @param {String} chatId
		* @returns chatId
		...
		* By Bolaxd
		*/
		serve.sendaud = async (chatId, aud, ptt = false, quoted = '', opts = {}) => serve.sendMessage(chatId, {audio: { url: aud }, ptt: ptt}, {quoted}, ephe, opts)
		/* 
		* Send Document
		* @param {String} chatId
		* @param {String} text
